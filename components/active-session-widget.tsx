...
export function ActiveSessionWidget({ onHoursUpdated }: { onHoursUpdated: () => void }) {
  ...
  return (
    <div className="flex items-center justify-between bg-[#111] border border-emerald-500/20 rounded-xl p-3 animate-in fade-in slide-in-from-top-2">
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center">
          <Timer className="w-4 h-4 text-emerald-400 animate-pulse" />
        </div>
        <div className="flex flex-col">
          <span className="text-[10px] uppercase tracking-widest font-bold text-emerald-400/80">Working: {activeSession.task}</span>
      ...
      <button 
        ...
        className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 px-4 py-2 rounded-lg text-[10px] uppercase tracking-widest font-bold transition-all disabled:opacity-50"
      >
        {isClockingOut ? (
          <Loader2 className="w-3 h-3 animate-spin" />
        ) : (
