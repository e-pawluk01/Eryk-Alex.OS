...
export function EditTopicDialog({ topic, onUpdate, onDelete }: EditTopicDialogProps) {
  ...
  const saveCustomColor = (color: string) => {
    ...
    if (editingColor) {
      if (customColors.includes(editingColor)) {
        ...
        localStorage.setItem(`custom_colors_${topic.context}`, JSON.stringify(newColors));
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("study_topics").update(updates).eq("id", topic.id);
  ...
  return (
    <>
      <button 
        ...
        className="opacity-0 group-hover:opacity-100 p-1.5 rounded bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 text-white/50 hover:text-white transition-all cursor-pointer absolute top-2 right-2 z-20 shadow-lg"
      >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative bg-zinc-950/80 backdrop-blur-3xl border border-white/5 rounded-2xl w-[90%] max-w-lg shadow-2xl shadow-black/50 animate-in fade-in zoom-in-[0.98] duration-300 slide-in-from-bottom-4">
            ...
            <div className="flex items-center justify-between px-6 py-5">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40">Edit Topic</span>
              <button 
                onClick={() => setIsOpen(false)} 
            ...
            <form onSubmit={handleSubmit} className="px-6 pb-6 flex flex-col gap-6">
              ...
              <div className="mt-2 flex justify-between items-center">
                <button
                  ...
                  disabled={isSubmitting}
                  className="px-4 py-2.5 bg-red-500/10 text-red-500 hover:bg-red-500/20 font-bold uppercase tracking-widest text-[10px] rounded-full transition-all disabled:opacity-30 flex items-center gap-2"
                >
                  <Trash2 className="w-3 h-3" />
                  Delete
                </button>
